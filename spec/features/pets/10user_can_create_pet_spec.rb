...
RSpec.describe "pet create ", type: :feature do
  ...
  it "can create a pet on pets index" do
    ...
    expect(page).to have_content("Remy has been created!")
  ...
  it "can create a pet on shelter show" do
    ...
    expect(page).to have_content("You must fill out Image")
    expect(current_path).to eq("/shelters/#{@shelter1.id}/pets/new")

    fill_in "name", with: "Remy"
    fill_in "Image", with: "imgage url"
    click_button "Create Pet"

    expect(page).to have_content("You must fill out Age")
    expect(current_path).to eq("/shelters/#{@shelter1.id}/pets/new")
    ...
    expect(page).to have_content("You must fill out Name")
    expect(current_path).to eq("/shelters/#{@shelter1.id}/pets/new")

    click_button "Create Pet"
    expect(page).to have_content("You must fill out Name, Age, Image")
    expect(current_path).to eq("/shelters/#{@shelter1.id}/pets/new")
